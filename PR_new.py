...
tau_interp = interp1d(tau_dat[:,0],tau_dat[:,1],fill_value="extrapolate")
tau_prime_interp = interp1d(tau_prime_dat[:,0],tau_prime_dat[:,1],fill_value="extrapolate")
tau_primeprime_interp = interp1d(tau_primeprime_dat[:,0],tau_primeprime_dat[:,1],fill_value="extrapolate")
...
def set_ICs(x,k):
    x_start = -22
    delta_CDM, v_CDM, delta_b, v_b, Phi, Theta_0, Theta_1 = TC.setup_for_post_TC(x_start,x,k)[-1]
...
#------------------------------
x_switch1 = -8
x_arr1 = np.linspace(x_switch1,-5,1000)
x_start = -22
x_tc1 = np.linspace(x_start,x_switch1,1000)
x_arrs_ = np.concatenate([x_tc1,x_arr1]) 
 
k_vals = np.logspace(2,4,1000)
...
for i in range(len(k_vals)):
    ...
    x_switch = TC.TC_end(k_new)
    ICs = set_ICs(x_switch,k_new)
    x_array = np.linspace(x_switch,-1,1000)
    x_start = -22

    P_R = odeint(dY_dx, ICs, x_array, args=(k_new,))


    T_C = TC.setup_for_post_TC(x_start,x_switch,k_new)
    x_T_C = np.linspace(x_start,x_switch,1000)
    x_array_c = np.concatenate([x_T_C,x_array]) 
    d_b_k = np.concatenate([T_C[:, 2],P_R[:, 2]])
    d_m_k = np.concatenate([T_C[:, 0],P_R[:, 0]])
    db_k_array_mod[i] = d_b_k
    dm_k_array_mod[i] = d_m_k
    x_k_array_mod[i] = x_array_c

file = open("db_k_array_new2.dat", "wb")
pickle.dump(db_k_array_mod, file)
file.close()

file2 = open("dm_k_array_new2.dat", "wb")
pickle.dump(dm_k_array_mod, file2)
file2.close()


file_x = open("x_k_array_new2.dat", "wb")
pickle.dump(x_k_array_mod, file_x)
file_x.close


db_k_array_shifted = np.zeros((len(k_vals), len(x_arrs_)))

for j in range(len(k_vals)):
    db_x_interp = interp1d(x_k_array_mod[j],db_k_array_mod[j])
    db_k_array_shifted[j] = db_x_interp(x_k_array_mod[0])
    
file_s = open("db_k_array_new2.dat", "wb")
pickle.dump(db_k_array_shifted, file_s)
file_s.close    

file_k = open("k_array_new2.dat", "wb")
